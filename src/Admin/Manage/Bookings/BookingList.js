...
import { Button, Table, Form, Modal, FormGroup, ModalBody } from 'react-bootstrap';
...
class BookingList extends Component {
    ...
    updateStatus = () => {
        instance.put(`api/Booking/${this.state.id}`)
            .then(res => {
                if (res) {
                    this.setState({
                        showUpdate: false
    ...
    deleteBooking = () => {
        instance.delete(`api/Booking/${this.state.id}`)
            .then(res => {
                if (res) {
                    const bookings = this.state.bookings.filter(item => item.id != this.state.id);
                    this.setState({ bookings });
    ...
    notify = (e) => this.toastId = toast("Đặt phòng thành công!", {
        transition: Slide,
        closeButton: true,
        autoClose: 3000,
    ...
    notify1 = (e) => this.toastId = toast("Đã cọc tiền!", {
        transition: Slide,
        closeButton: true,
        autoClose: 3000,
        position: 'bottom-center'
    ...
    notify2 = () => this.toastId = toast("Đã xóa thông tin đặt phòng!", {
        transition: Slide,
        closeButton: true,
        autoClose: 3000,
        position: 'bottom-center'
                        ...
                        <CSSTransition component="div" className="TabsAnimation"
                            ...
                            <div>
                                        ...
                                        {this.state.bookings.map((booking) => (
                                            <tr v-for="item in tableItems" key={booking.id}>
                                                ...
                                                <td className='text-center'>
                                                    <Input id='status' style={{ backgroundColor: 'green', borderColor: 'green' }} defaultChecked={booking.status == true} value={booking.status} type='checkbox' onChange={(e) => this.onChangeStatus()} />
                                                </td>
                                                <td className='text-center'>{moment(booking.createDate).format('DD-MM-YYYY')}</td>
                                                <td className='right'>
                                                    <Row style={{ float: 'right' }}>
                                                        <Col>
                                                            <Button style={{ border: 'none' }} onClick={(e) => this.handleShowUpdate(booking.id)} variant="outline-info">
                                                                <FaSave />
                                                            </Button>
                                                        </Col>
                                                        <Col>
                                                            <Button style={{ border: 'none' }} variant="outline-danger" onClick={(e) => this.handleShowDelete(booking.id)}>
                                                                <FaTrashAlt />
                                                            </Button>
                                                        </Col>
                                                    </Row>
                                ...
                                >
                                    ...
                                    <Modal.Body>
                                        Bạn có muốn xóa thông tin đặt phòng này ra khỏi danh sách không?
                                    </Modal.Body>
                                    <Modal.Footer>
                                        <Button variant="danger" onClick={(e) => this.deleteBooking(this.state.id)}>
                                            Có
                                        </Button>
                                ...
                                >
                                    ...
                                    <Modal.Footer>
                                        <Button variant="danger" onClick={(e) => this.updateStatus(this.state.id)}>
                                            Có
                                        </Button>
                                        <Button variant="secondary" onClick={(e) => this.handleCloseUpdate()}>Không</Button>
                                    </Modal.Footer>
                                </Modal>
                            </div>
                        </CSSTransition>
