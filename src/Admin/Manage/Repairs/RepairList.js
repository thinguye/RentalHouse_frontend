...
export default class RepairList extends Component {
  ...
  handleStatusChange = (repairId, status) => {
    if (status !== "Đã Hoàn Thành") {
      const id = repairId;
      instance.put(`api/RequestRepair/${id}`).then((res) => {
        if (res) {
                  ...
                  <tbody style={{ borderTop: "1px solid blue" }}>
                    {this.state.repairs.map((repair) => (
                      <tr v-for="item in tableItems" key={repair.id}>
                        <td>{repair.roomName}</td>
                        <td>{repair.description}</td>
                        <td>{moment(repair.date).format("DD-MM-YYYY")}</td>
                        <td>
                          <Button
                            variant="none"
                            style={{ color: this.setColor(repair.status) }}
                            onClick={(e) =>
                              this.handleStatusChange(repair.id, repair.status)
                            }
                          >
                            <FaCircle />
                            {"   "}
                        ...
                        <td>
                          <Button
                            ...
                            onClick={(e) => this.handleShow(repair.id)}
                          >
                            <FaTrashAlt />
                          </Button>
                        </td>
