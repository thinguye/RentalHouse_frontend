...
class BillList extends Component {
    ...
    updateStatus = () => {
        instance.put(`api/Bill/${this.state.id}`)
            .then(res => {
                if (res) {
                    this.setState({
                        showUpdate: false
    ...
    deleteBill = () => {
        instance.delete(`api/Bill/${this.state.id}`)
            .then(res => {
                if (res) {
                    const bills = this.state.bills.filter(item => item.id != this.state.id);
                    this.setState({ bills });
    ...
    notify1 = (e) => this.toastId = toast("Đã thanh toán!", {
        transition: Slide,
        closeButton: true,
        autoClose: 3000,
    ...
    notify2 = () => this.toastId = toast("Đã xóa hóa đơn!", {
        transition: Slide,
        closeButton: true,
        autoClose: 3000,
        position: 'bottom-center'
                        ...
                        <CSSTransition component="div" className="TabsAnimation"
                            ...
                            <div>
                                        ...
                                        {this.state.bills.map((bill) => (
                                            <tr v-for="item in tableItems" key={bill.id}>
                                                ...
                                                <td className='text-center'>
                                                    <Input style={{ backgroundColor: 'green', borderColor: 'green' }} defaultChecked={bill.is_Pay == true} value={bill.is_Pay} type='radio'/>
                                                </td>
                                                <td className='right'>
                                                    <Row style={{ float: 'right' }}>
                                                        <Col>
                                                            <Button style={{ border: 'none' }} onClick={(e) => this.handleShowUpdate(bill.id)} variant="outline-info">
                                                                <FaSave />
                                                            </Button>
                                                        </Col>
                                                        <Col>
                                                            <Button style={{ border: 'none' }} variant="outline-danger" onClick={(e) => this.handleShowDelete(bill.id)}>
                                                                <FaTrashAlt />
                                                            </Button>
                                                        </Col>
                                                    </Row>
                                ...
                                >
                                    ...
                                    <Modal.Footer>
                                        <Button variant="danger" onClick={(e) => this.deleteBill(this.state.id)}>
                                            Có
                                        </Button>
