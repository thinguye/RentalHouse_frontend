...
export default class OldGuests extends Component {
    ...
    componentDidMount() {
        instance.get(`api/Customer/old+customers`)
            .then(res => {
    ...
    deleteGuest = () => {
        instance.delete(`api/Customer/${this.state.id}`).then(res => {
            if (res) {
                const guests = this.state.guests.filter(item => item.id != this.state.id);
                this.setState({ guests });
                                        ...
                                        <tr v-for="item in tableItems" key={guest.id}>
                                            ...
                                            <td>
                                                <Row>
                                                    ...
                                                    <Col>
                                                        <Button style={{border:'none'}} variant="outline-danger" onClick={(e) => this.handleShowDelete(guest.id, guest.name)}>
                                                            <FaTrashAlt />
                                                        </Button>
                                                    </Col>
                                                </Row>
